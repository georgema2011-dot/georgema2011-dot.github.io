...
export default function Home(){
 ...
 return <main id="top">
  ...
  <section className="photos" id="photography"><div className="shell"><Head eyebrow="Project 06 · Personal study" title="PHOTOGRAPHY JOURNEY" lead="Observing the hush, bustle and traces of life across changing landscapes."/><div className="gallery-intro reveal"><p>Fragments from Scandinavia, Copenhagen and Hong Kong—quiet edges, crowded light, and places carrying the evidence of time.</p><span>Archive / 001—012</span></div><div className="photo-gallery">{photoArchive.map(([file,meta,title,alt],index)=><figure className="gallery-shot reveal" key={file}><img src={`/photo-dump/web/${file}`} alt={alt} loading="lazy"/><figcaption><b>{String(index+1).padStart(3,"0")}</b><span>{title}</span><small>{meta}</small></figcaption></figure>)}</div><div className="contact-sheet reveal" aria-hidden="true"><span>PHOTO LOG</span><i/><i/><i/><i/><i/><i/><small>GM—06 / SINGAPORE</small></div></div></section>
